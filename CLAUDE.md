...
make build-py CHAPKIT_VERSION=1.0.0
```
...
- `workflow_dispatch` — manual with `chapkit_version` input (PyPI version, e.g. `1.0.0`)
...
- `fix: fall back CHAPKIT_VERSION to github.ref_name on tag pushes`
...
- Don't hardcode a chapkit version into a `RUN` line — it's `ARG CHAPKIT_VERSION` for a reason. Bumping the pinned default belongs in a single arg edit.
